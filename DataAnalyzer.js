...
function DataAnalyzer() {
    ...
    function setArrayFromString(str){
        ...
        for (var i=0;i<str.length;i++){
            //if (str.charCodeAt(i) != 32)        // space 제거 // 2019-07 요청에 의해 제거
    ...
    function getDataCheckResult() {
        ...
        _barcodeDataList.forEach(function(rowData,rowIndex){
            ...
            // 데이터가 있는지 확인하고 필수 여부에 따라 result 표시
            if (!ex_00) {
                result = false;
    ...
    // 구분자 기준으로 split (Header와 Trailer 제거)
    ...
    function chkValidDate(arrData) {
        ...
        if ((year % 4 == 0) && ((year % 100 != 0) || (year % 400 == 0))) {
            if (month == 2) {
                if (day <= m_last_day[month-1] + 1) {
                    return true;
                }
            } else {
                if (day <= m_last_day[month-1]) {
                    return true;
                }
            }
        } else {
    ...
    // 한 바코드 데이터 (배열데이터)에서 구분자 기준으로 맨 앞 데이터를 가져온다
    function getFirstBlockData(arrData){
        var cutIndex = -1;
        arrData.some(function(v,i){
            if (v == 29){
                cutIndex = i;
                return true;
            }
    ...
    // 한 바코드 데이터 (배열데이터)에서 구분자 기준으로 맨 뒤 데이터를 가져온다
    ...
    // 배열데이터에서 허용되지 않는 ASCII코드가 있는지 확인
